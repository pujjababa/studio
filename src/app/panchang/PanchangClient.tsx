...
export function PanchangClient() {
  ...
  const [panchangDetails, setPanchangDetails] = useState<PanchangDetails | null>(null);
  ...
  const getErrorMessage = (e: any) => {
    let description = 'Failed to get a response from the API. Please try again.';
    if (e.message?.includes('503') || e.message?.includes('overloaded')) {
        description = 'The Panchang API is currently overloaded. Please try again in a moment.';
    } else if (e.message) {
        return e.message;
    }
    return description;
  ...
  async function getPanchangDetails(inputDate: Date) {
      ...
      try {
        ...
        setPanchangDetails(result);

      } catch (e: any) {
        console.error('Error fetching panchang:', e);
        const description = getErrorMessage(e);
        setError(`Failed to fetch Panchang data. ${description}`);
        toast({
          variant: 'destructive',
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
           ...
           <CardDescription className='text-xs pt-2'>(Data provided by ProKerala API)</CardDescription>
        </CardHeader>
